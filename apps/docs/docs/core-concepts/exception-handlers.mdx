...
```ts
import { OnException, ExceptionHandler, InteractionContext } from '@nodecord/core';

@OnException(DatabaseError)
export class DatabaseErrorHandler implements ExceptionHandler {
    handle(exception: unknown, ctx: InteractionContext): void {
...
- `ctx` — the `InteractionContext` for the command that failed
